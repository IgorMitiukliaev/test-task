...
import {clearActiveTags, setActiveTags} from '../../actions'
    ...
    if (activeTags.length > 0) {
        ...
        return (
            <div style={{margin: '20px 5px 10px 5px'}}>
                Filtered by tags: {tagLabel} <span onClick={() => setActiveTags()}>(clear)</span>
                <hr/>
            </div>
        )
    }
...
const mapDispatchToProps = {setActiveTags};
